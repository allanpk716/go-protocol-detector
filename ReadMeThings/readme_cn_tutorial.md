...
SFTP 协议运行在 SSH 协议之上，采用三层递进检测策略：

1. **TCP连接测试**：建立到目标端口的 TCP 连接，验证端口可达性
2. **SSH协议识别**：读取服务器返回的 SSH Banner（如 `SSH-2.0-OpenSSH_8.2p1`），确认是 SSH 服务
3. **SFTP子系统查询**：建立 SSH 客户端连接，通过 subsystem 请求检测 SFTP 子系统是否可用

这种无认证检测方式无需提供凭据即可判断 SFTP 服务可用性，对协议扫描场景最为实用。

## 如何分析 RustDesk

RustDesk 是一款开源的远程桌面软件，其服务端包含以下组件：

* **HBBS（信令/注册服务器）**：负责 ID 注册、心跳保活和 TCP 打洞，默认端口 21116
* **HBBR（中继服务器）**：负责流量中继转发，默认端口 21117

RustDesk 的通信协议基于 Protobuf 编码，检测方式如下：

* **HBBS 检测（端口 21115/21116）**：构造 `TestNatRequest` 或 `RegisterPk` Protobuf 消息发送给服务器，通过分析响应中的特征字段（如 `TestNatResponse` 的端口信息、`RegisterPkResponse` 的结果字段）确认 HBBS 服务
* **HBBR 检测（端口 21117）**：发送 `RequestRelay` 消息，服务器接受连接后会保持等待配对，基于此行为特征可靠识别 HBBR 服务

由于采用 Protobuf 二进制编码，相比明文协议检测需要额外的编解码处理，但也因此误报率较低。
